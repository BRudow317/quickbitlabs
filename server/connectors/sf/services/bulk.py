...
def call_salesforce(
    ...
    if result.status_code >= 300:
        raise Exception(result.url, result.status_code, "Salesforce API", result.content)
...
class SFBulkType:
    ...
    def _create_job(self,
        ...
        * external_id_field -- unique identifier field for upsert operations
        ...
        result = self.sf_session.call_salesforce(
            'POST', url, data=json.dumps(payload, allow_nan=False)
        )
    ...
    def _add_batch(
        ...
        result = self.sf_session.call_salesforce('POST', url, data=data_)
        return result.json(object_pairs_hook=OrderedDict)
    ...
    def _get_batch_results(
        ...
        if operation in ('query', 'queryAll'):
            for batch_result in result.json():
                ...
                batch_query_result = self.sf_session.call_salesforce(
                    'GET', url_query_results
                ).json()
    ...
    def _get_batch_request_with_batch_results(self,
        ...
        batch_request = self.sf_session.call_salesforce('GET', url)
        ...
        for batch_result in batch_results:
            for idx, i in enumerate(batch_result):
                record: dict[str, Any] = batch_request.json()[idx]
                flattened_request_dict: List[dict[str, Any]] = [{
                                              k + '.' + list(cast(dict[str, Any], v).keys())[0]: cast(dict[str, Any], v).get(
                                                  list(cast(dict[str, Any], v).keys())[0]
                                                  )
                                              } if isinstance(v,
                                                              dict
                                                              ) else {
        ...
        * batch_size -- number of records to assign for each batch in the job
        ...
        if operation not in ('query', 'queryAll'):
            ...
            if batch_status['state'] == 'Failed':
                raise Exception('',batch_status['state'],batch_status['jobId'],batch_status['stateMessage'])
