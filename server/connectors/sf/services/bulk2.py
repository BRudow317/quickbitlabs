...
class SfBulk2Type:
    ...
    def _upload_data(self, operation: Operation, data: str | tuple[int, str], column_delimiter: ColumnDelimiter = ColumnDelimiter.COMMA, line_ending: LineEnding = LineEnding.LF, external_id_field: str | None = None, wait: int = 5) -> dict[str, int]:
    ...
    def insert(self, csv_file: str | None = None, records: list[dict[str, str]] | None = None, batch_size: int | None = None, concurrency: int = 1, column_delimiter: ColumnDelimiter = ColumnDelimiter.COMMA, line_ending: LineEnding = LineEnding.LF, quoting: QUOTING_TYPE = csv.QUOTE_MINIMAL, wait: int = 5) -> list[dict[str, int]]:
        return self._upload_file(Operation.insert, csv_file=csv_file, records=convert_dict_to_csv(records, ColumnDelimiter(column_delimiter), LineEnding(line_ending), quoting), batch_size=batch_size, column_delimiter=column_delimiter, line_ending=line_ending, quoting=quoting, concurrency=concurrency, wait=wait)
