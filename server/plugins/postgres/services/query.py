...
from server.plugins.postgres.postgres_utils.type_converter import pg_to_python_type, pg_source_type, PYTHON_TO_PG
from server.plugins.postgres.services.table_ops import PgObjType
...
class PgQuery:
    ...
    def table(self, name: str, pg_schema: str | None = None) -> PgObjType:
        """BaseSchema-aware table accessor. Use this instead of __getattr__ when pg_schema matters."""
        return PgObjType(table_name=name, engine=self._engine, metadata=self._metadata, pg_schema=pg_schema)

    # ── BaseSchema Discovery ──
    ...
    def describe(self, table_name: str, pg_schema: str | None = None) -> EntityModel:
        """Reflect a single table into the universal BaseTable model."""
        sa_table = SATable(table_name, self._metadata, autoload_with=self._engine, schema=pg_schema)

        columns = []
        for col in sa_table.columns:
            columns.append(FieldModel(
                source_name=col.name,
                python_type=pg_to_python_type(col.type),
    ...
    def describe_all(self, pg_schema: str = 'public') -> CatalogModel:
        """Reflect an entire Postgres schema into a universal BaseSchema."""
        inspector = inspect(self._engine)
    ...
    def create_table(self, table: EntityModel   , pg_schema: str | None = None) -> None:
        """Create a Postgres table from a BaseTable. Existing tables are left unchanged."""
        if pg_schema:
        ...
        for col in table.columns:
            sa_type_cls = PYTHON_TO_PG.get(col.python_type, String)
    ...
    def add_column(self, table_name: str, column: BaseColumn, pg_schema: str | None = None) -> None:
    ...
    def query_iter(self, sql: str, params: dict[str, Any] | None = None, batch_size: int = 10000) -> Records:
