...
export type BadgeCorner = "bottom-right" | "bottom-left" | "top-right" | "top-left" | "all";
...
export const Badge: React.FC<BadgeProps> = ({
  ...
  const frameClass = ["absolute inset-0 pointer-events-none z-0", roundedClass, clipClass, variant === "outline" ? "bg-[var(--bdg-color)]" : "bg-white/[0.08]"].filter(Boolean).join(" ");
  ...
  const innerClass = [
    ...
    INNER_SIZE[size], roundedClass, clipClass,
    variant === "solid" ? "bg-[var(--bdg-color)] text-black" : variant === "outline" ? "bg-black text-[var(--bdg-color)]" : "text-[var(--bdg-color)]",
    glow ? "bdg-glow" : "",
  ].filter(Boolean).join(" ");

  const ghostStyle = variant === "ghost" ? { backgroundColor: "color-mix(in srgb, var(--bdg-color) 12%, #000)" } : undefined;

  const dotAnimClass = dot === "pulse" ? "bdg-dot-pulse" : dot === "flicker" ? "bdg-dot-flicker" : "";

  return (
    <span
      className={["relative inline-flex p-px align-middle", roundedClass, className].filter(Boolean).join(" ")}
      style={{ "--bdg-color": resolvedColor, "--bdg-corner-size": `${cornerSize}px`, ...style } as React.CSSProperties}
      {...props}
    >
      <span className={frameClass} aria-hidden="true" />
      <span className={innerClass} style={ghostStyle}>
        {dot !== "none" && (
          <span className={["rounded-full bg-[var(--bdg-color)] inline-block shrink-0", DOT_SIZE[size], dotAnimClass].filter(Boolean).join(" ")} aria-hidden="true" />
        )}
        {children}
      </span>
    </span>
