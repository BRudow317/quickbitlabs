...
function toRgba(color: string, alpha: number): string {
  ...
  if (t.startsWith("rgba(")) { if (alpha === 1) return t; return t.replace(/,\s*[\d.]+\s*\)$/, `,${alpha})`); }
...
interface HexCell { cx: number; cy: number; verts: [number, number][]; }
interface HexEdge { x1: number; y1: number; x2: number; y2: number; v1: number; v2: number; dx: number; dy: number; length: number; }
interface HexGraph { verts: { x: number; y: number }[]; edges: HexEdge[]; vertToEdges: number[][]; }

function buildGrid(width: number, height: number, r: number): { cells: HexCell[]; graph: HexGraph } {
  ...
  const vKey = (x: number, y: number) => `${Math.round(x * 10)},${Math.round(y * 10)}`;
  const getVert = (x: number, y: number): number => {
    const k = vKey(x, y);
    if (!vertMap.has(k)) { vertMap.set(k, gVerts.length); gVerts.push({ x, y }); }
    return vertMap.get(k)!;
  };
  for (let col = -1; col < cols; col++) {
    for (let row = -1; row < rows; row++) {
      const cx = col * horizSpacing + r;
      const cy = row * vertSpacing + (col % 2 !== 0 ? vertSpacing / 2 : 0) + h / 2;
  ...
  for (const cell of cells) {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      gEdges.push({ x1, y1, x2, y2, v1: vi, v2: vj, dx: (x2 - x1) / len, dy: (y2 - y1) / len, length: len });
  ...
  const vertToEdges: number[][] = gVerts.map((_, i) => [...(vteMap.get(i) ?? [])]);
  return { cells, graph: { verts: gVerts, edges: gEdges, vertToEdges } };
}

interface Beam { edgeIdx: number; forward: boolean; t: number; speed: number; tailPx: number; trailLen: number; trail: [number, number][]; }

function beamHeadPos(beam: Beam, edges: HexEdge[]): [number, number] {
  const e = edges[beam.edgeIdx];
  return beam.forward ? [e.x1 + e.dx * beam.t, e.y1 + e.dy * beam.t] : [e.x2 - e.dx * beam.t, e.y2 - e.dy * beam.t];
}

function spawnBeam(graph: HexGraph, hexSize: number, speed: number, tailLengthPx: number): Beam | null {
  const { verts, edges, vertToEdges } = graph;
  const topVIdxs: number[] = [];
  verts.forEach((v, i) => { if (v.y < hexSize * 2.5) topVIdxs.push(i); });
  ...
  for (const vIdx of topVIdxs) {
    ...
    return { edgeIdx: eIdx, forward, t: 0, speed: speed * (0.75 + Math.random() * 0.5), tailPx: tailLengthPx * (0.7 + Math.random() * 0.6), trailLen: 0, trail: [] };
  }
  return null;
}
...
function stepBeam(beam: Beam, graph: HexGraph, canvasH: number): boolean {
  ...
  const candidates = (vertToEdges[arrV] ?? []).filter((ei) => ei !== beam.edgeIdx).map((ei) => {
    const ce = edges[ei];
  ...
  const lateral = [...downward, ...horizontal];
  let pool: typeof candidates;
  if (lateral.length === 0) pool = candidates;
  else if (downward.length > 0 && horizontal.length > 0) pool = Math.random() < 0.55 ? horizontal : downward;
  else pool = lateral;
...
interface HexCanvasProps {
  hexColor: string; hexBorderColor: string; hexSize: number; borderWidth: number;
  hoverEffect: boolean; hoverColor: string; hoverBorderColor: string;
  borderGlowEffect: boolean; borderGlowColor: string; borderGlowRadius: number;
  beamEffect: boolean; beamColor: string; beamGlowColor: string;
  maxBeams: number; beamSpeed: number; beamLength: number; beamSpawnProbability: number;
}

function HexCanvas({ hexColor, hexBorderColor, hexSize, borderWidth, hoverEffect, hoverColor, hoverBorderColor, borderGlowEffect, borderGlowColor, borderGlowRadius, beamEffect, beamColor, beamGlowColor, maxBeams, beamSpeed, beamLength, beamSpawnProbability }: HexCanvasProps) {
  ...
  useEffect(() => {
    if (!hoverEffect) { hoveredRef.current = -1; return; }
    ...
    const onMove = (ev: MouseEvent) => {
      ...
      if (mx < 0 || my < 0 || mx > rect.width || my > rect.height) { hoveredRef.current = -1; return; }
      let best = -1;
      let bestD = inscribed;
  ...
  useEffect(() => {
    ...
    const drawBackground = (bgCtx: CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D, W: number, H: number) => {
      ...
      if (hexColor !== "transparent") { bgCtx.fillStyle = hexColor; bgCtx.fill(); }
      if (borderGlowEffect) { bgCtx.shadowColor = borderGlowColor; bgCtx.shadowBlur = borderGlowRadius; }
      ...
      if (hov >= 0) {
        ...
        if (borderGlowEffect) { bgCtx.shadowColor = hBorder; bgCtx.shadowBlur = borderGlowRadius; }
    ...
    const rebuild = () => {
      ...
      try { bgRef.current = new OffscreenCanvas(bgW, bgH); } catch {
        const el = document.createElement("canvas");
        el.width = bgW; el.height = bgH;
        bgRef.current = el;
      }
      const bgCtx = bgRef.current.getContext("2d") as CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D | null;
    ...
    const render = () => {
      ...
      if (!bg) { rafRef.current = requestAnimationFrame(render); return; }
      const bgCtx = bg.getContext("2d") as CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D | null;
      if (!bgCtx) { rafRef.current = requestAnimationFrame(render); return; }
      ...
      if (beamEffect && graphRef.current.edges.length > 0) {
        if (Math.random() < beamSpawnProbability && beamsRef.current.length < maxBeams) {
        ...
        for (const beam of beamsRef.current) {
          ...
          if (beam.trail.length >= 2) {
            ...
            for (let j = 1; j < beam.trail.length; j++) ctx.lineTo(beam.trail[j][0], beam.trail[j][1]);
  ...
  }, [hexColor, hexBorderColor, hexSize, borderWidth, hoverEffect, hoverColor, hoverBorderColor, borderGlowEffect, borderGlowColor, borderGlowRadius, beamEffect, beamColor, beamGlowColor, maxBeams, beamSpeed, beamLength, beamSpawnProbability]);
...
export function Hexagons({
  ...
  return (
    <div className={`absolute inset-0 overflow-hidden ${className}`}>
      <HexCanvas hexColor={hexColor} hexBorderColor={hexBorderColor} hexSize={hexSize} borderWidth={borderWidth} hoverEffect={hoverEffect} hoverColor={hoverColor} hoverBorderColor={hoverBorderColor} borderGlowEffect={borderGlowEffect} borderGlowColor={borderGlowColor} borderGlowRadius={borderGlowRadius} beamEffect={beamEffect} beamColor={beamColor} beamGlowColor={beamGlowColor} maxBeams={maxBeams} beamSpeed={beamSpeed} beamLength={beamLength} beamSpawnProbability={beamSpawnProbability} />
      {overlay && (
        <>
